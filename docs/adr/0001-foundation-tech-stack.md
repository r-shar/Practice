...
- Relational data with migrations and types (therapists, clients, and whatever comes next)
...
One product decision came up early and is shaping the build: **this app is for therapists, not their clients.**

My sister's a therapist, and the pain points I've heard about are mostly on *her* side—scheduling, notes, billing, keeping track of clients. Clients need to exist in the system as people she works with, but they don't need accounts or a logged-in experience in v1. That distinction mattered more than I expected when I started thinking about auth.
...
**Only therapists are users.** If you can sign in via Clerk, you're a therapist. The app keeps a `Therapist` row keyed by `clerkId`. Clients are just records the therapist owns—name, email, phone. No `clerkId` on `Client`, no Clerk account for clients. The DB is basically a CRM for caseload management.
...
**Prisma schema split across files.** Models live in `prisma/therapist.prisma`, `prisma/client.prisma`, etc., with shared config in `schema.prisma`. Mostly so the schema doesn't turn into one giant file as the domain grows.

**Generated client is gitignored.** Prisma Client outputs to `src/generated/prisma`. Run `npm run db:generate` after cloning or schema changes.

**Prisma 7 + pg driver adapter.** Singleton client in `src/lib/db.ts` using `@prisma/adapter-pg`. That's what Prisma 7 expects for a direct Postgres connection.

**Everything scoped to a therapist.** Clients belong to one therapist via foreign key. I'm assuming future domain data follows the same boundary unless a later ADR says otherwise.

**Portal links for client-facing stuff (later, not now).** When a client needs to book a slot or fill out a form, I'd rather send them a signed, time-limited link than make them create an account. The platform isn't built for clients as logged-in users—but that doesn't mean they can never interact with it at all.
...
**One `User` table with roles (`THERAPIST | CLIENT`).**
Both personas sign in through Clerk, one identity model, role checks everywhere. I decided against this because I'm not building a client-facing app right now. Separate UX paths, client onboarding, role guards—all that complexity before there's even a client login experience to build. If I need client login later, I can add a nullable `Client.clerkId` without redesigning everything.
...
- Clients-as-records matches how I actually think about the product. Permissions are simple: you're a therapist, you see your clients.
- No client auth means no password resets or MFA flows for people who were never supposed to log in anyway.
...
- Clients can't self-serve until portal links exist.
...
- Portal-link design (`clientId`, purpose, expiry) when client scheduling is in scope.
